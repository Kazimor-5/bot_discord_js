  ...
  async run(msg, args) {
    ...
    if (args.command && !showAll) {
      if (commands.length === 1) {
        ...
        embed.setDescription(
          disambiguation(commands, 'commands').replace(/ or /g, ' ou ')
        );
      } else {
        embed.setDescription(
          `Cette commande n'existe pas. Utilisez ${msg.usage(
            null,
            msg.channel.type === 'dm' ? null : undefined,
            msg.channel.type === 'dm' ? null : undefined
          )} pour voir la liste de toutes les commandes`.replace(
            / or /g,
            ' ou '
          )
        );
      ...
      embed.setDescription(
        stripIndents`
					${oneLine`
						Pour lancer une commande, utilisez ${Command.usage(
              'command',
            ...
            null,
            null
          )} pour voir les informations détaillées de la commande spécifiée.

					__**Commandes disponibles :**__

					${groups
            .filter((grp) =>
              grp.commands.some((cmd) => !cmd.hidden && cmd.isUsable(msg))
            )
            .map(
              (grp) => stripIndents`
							・ **${grp.name}**
							${grp.commands
                .filter((cmd) => !cmd.hidden && cmd.isUsable(msg))
                .map(
                  (cmd) =>
